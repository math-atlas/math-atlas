...
#define NFP 9 /* fp count for sine  from taylor series */
...
double DoTiming(int N, int nkflop, int cachesize, int incX, int incY)
{
   ...
   if (nrep < 1) nrep = 1;
/*
 * allocate memory and flush the cache  
 */
   assert(incX==1);
   assert(incY==1);
 ...
 */
   ...
   if ( (lda & (lda-1)) == 0) 
      lda += tp;
   M = 4 * lda; /* that's the working set */
 ...
 */
{
   ...
   double mf = (N*NFP)/1000000.0; /* mfp for N element */
   double *tims;
   int n, i, j, k;
   FILE *fpout=NULL;
              ...
              int *cachesize, char **fnam, int *incX, int *incY)
{
   ...
   *nN = -1;
/*
 * Using default nkflops would exceed the memory to allocate for timing!!!!
 */
