...
#ifndef TEST_KERNEL
   #define TEST_KERNEL  FKO_IRK3AMAX 
#endif

#ifndef TRUSTED_KERNEL
   #define TRUSTED_KERNEL good_irk3amax
#endif
...
static int good_irk3amax(const int M, const TYPE s0, TYPE *A0, const int lda)
/*
...
/*
 * need to allocate 4*lda where lda >= N ... 
 */
   Ag = getvec(npad, padval, 4*lda, incX, FAy, MAy);
   At = dupvec(npad, 4*lda, Ag, incX , FAy, MAy);
   ...
   iret = CheckY(npad, padval, 4*lda, Ag, incY, At, incY);
