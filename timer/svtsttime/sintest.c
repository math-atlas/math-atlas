...
#ifndef TEST_KERNEL_SIN
   #define TEST_KERNEL_SIN  FKO_SINE 
#endif

#ifndef TRUSTED_KERNEL_SIN
   #define TRUSTED_KERNEL_SIN good_kernel_sin
#endif
...
#ifdef TREAL
#ifdef SREAL
void good_kernel_sin(const int N, const TYPE *X, TYPE *Y)
{
   int i;
   float x, x1, y, z, v, r;
   const float s1 = -1.6666667163e-1, s2 = 8.3333337680e-3,
               s3 = -1.9841270114e-4, s4 = 2.7557314297e-6, 
               s5 = -2.5050759689e-8, s6 = 1.5896910177e-10, 
               lmark = 7.4505806e-9, half = 5.0e-1, fzero = 0.0e0;
   for (i=0; i < N; i++, X++, Y++)
   {
      x = *X;
      y = *Y;
