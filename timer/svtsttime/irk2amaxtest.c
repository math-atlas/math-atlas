...
#ifndef TEST_KERNEL
   #define TEST_KERNEL  FKO_IRK2AMAX 
#endif

#ifndef TRUSTED_KERNEL
   #define TRUSTED_KERNEL good_irk2amax
#endif
...
static int good_irk2amax(const int M, const TYPE s0, TYPE *A0, const int lda)
/*
...
int DoTest(int N, int incX, int incY)
{
   ...
   TYPE *Ag, *At, *a0, s0;
   int lda, tp, M, gimax, timax;
/*
 * Figure out suitable lda for this testing
 * NOTE: lda must be multiple of 8 for single precision and multiple of 4 for
 * double precision sothat it can be multiple of 32 byte.
 */
#ifdef SREAL   
...
#endif
/*
 * Here, we consider N as the M of each vector 
 */
...
/*
 * need to allocate 2*lda where lda >= N ... 
 */
   Ag = getvec(npad, padval, 3*lda, incX, FAy, MAy);
   At = dupvec(npad, 3*lda, Ag, incX , FAy, MAy);
   
   s0 = dumb_rand(); 
   
   a0 = Ag + (npad SHIFT); 
   gimax = TRUSTED_KERNEL(M, s0, a0, lda);
   
   a0 = At + (npad SHIFT);
   timax = TEST_KERNEL(M, s0, a0, lda);
   
   iret = CheckY(npad, padval, 3*lda, Ag, incY, At, incY);
   
   if (gimax != timax)
   {
      iret = 1;
      fprintf(stderr, 
             "   IAMAX ERROR %d: correct index=%d, computed index=%d \n",
             __LINE__, gimax, timax);
   }
   FA_free(Ag, FAy, MAy);
   FA_free(At, FAy, MAy);
   return(iret);
