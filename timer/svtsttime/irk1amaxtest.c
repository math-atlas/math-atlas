...
#ifndef TEST_KERNEL
   #define TEST_KERNEL  FKO_IRK1AMAX 
#endif

#ifndef TRUSTED_KERNEL
   #define TRUSTED_KERNEL good_irk1amax
#endif
...
int good_irk1amax(const int M, const TYPE s0, TYPE *A0, const int lda)
/*
...
#endif
/*
 * here, we consider N as M, length of each vector  
 */
 ...
 */
   Ag = getvec(npad, padval, 2*lda, incX, FAy, MAy);
   At = dupvec(npad, 2*lda, Ag, incX , FAy, MAy);
   ...
   iret = CheckY(npad, padval, 2*lda, Ag, incY, At, incY);
