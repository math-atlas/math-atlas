...
Mjoin( Mjoin( ATL_i, PRE ), tamax ) \
                  (      N,     X, iX                                )
#elif defined(IFKO)
   int ATL_UIAMAX( int N, TYPE *X, int iX); 
#define test_amax(       N,     X, iX                                ) \
ATL_UIAMAX(               N,     X, iX                             )
#else
#define test_amax(       N,     X, iX                                ) \
...
#ifdef IFKO
   void ATL_USCAL( int N, TYPE al, TYPE *X, int iX);
...
#ifdef IFKO
   void ATL_UAXPY( int N, TYPE al, TYPE *X, int iX, TYPE *Y, int iY );
#define test_axpy(       N, al, X, iX, Y, iY                         ) \
ATL_UAXPY(               N, al, X, iX, Y, iY                         )
#else
#define test_axpy(       N, al, X, iX, Y, iY                         ) \
...
#ifdef IFKO
   void ATL_UCOPY( int N, TYPE *X, int iX, TYPE *Y, int iY );
#define test_copy(       N,     X, iX, Y, iY                         ) \
ATL_UCOPY(               N,     X, iX, Y, iY                         )
#else
#define test_copy(       N,     X, iX, Y, iY                         ) \
...
#ifdef IFKO   
   void ATL_USWAP( int N, TYPE *X, int iX, TYPE *Y, int iY );
#define test_swap(       N,     X, iX, Y, iY                         ) \
ATL_USWAP(               N,     X, iX, Y, iY                         )
#else
#define test_swap(       N,     X, iX, Y, iY                         ) \
...
/*
 *======================================================================
 * Majedul: aligned malloc function and vars needed to time ifko_blas
 * To maintain fixed alignment, N is needed to be multiple of some sizes
 *======================================================================
 */
/* Right now, I need fixed alignment: 32byte or 16byte for all ptr*/
#if defined (ALIGN32)
int FA=32, MA=0;
#elif defined (ALIGN16)
int FA=16, MA=0;
#else /* default 32 byte for now*/
int FA=32, MA=0;
#endif
...
/*
 * Wrapper function for the Aligned malloc
 */
void *fk_malloc(size_t size)
{
   return FA_malloc(size, FA, MA);
}
void *fk_calloc(size_t n, size_t size)
{
   return FA_calloc(n, size, FA, MA);
}

void fk_free(void *ptr)
{
   return FA_free(ptr, FA, MA);
}
 ...
 */
   l2ret = ATL_flushcache( CACHESIZE );
#ifdef IFKO
   X     = (TYPE *)fk_malloc( ATL_MulBySize( N ) * aincX );
#else
 ...
 */
   l2ret = ATL_flushcache( 0 );
#ifdef IFKO
   if( !( TEST ) ) { fk_free( X ); return( ATL_rzero ); }
#else
 ...
 */
   l2ret = ATL_flushcache( CACHESIZE );
#ifdef IFKO
   X = (TYPE *)fk_malloc( ATL_MulBySize( N ) * aincX * 2 );
#else
 ...
 */
#ifdef IFKO
   XD = (TYPE *)fk_malloc( ATL_MulBySize( N ) );
   if( XD == NULL ) { fk_free( X ); return( ATL_rnone ); }
#else
...
#ifdef IFKO
   X  = (TYPE *)fk_malloc( ATL_MulBySize( N ) * aincX     );
   Y  = (TYPE *)fk_malloc( ATL_MulBySize( N ) * aincY * 2 );
#else
   X  = (TYPE *)malloc( ATL_MulBySize( N ) * aincX     );
 ...
 */
   l2ret  = ATL_flushcache( 0 );
#ifdef IFKO
   if( !( TEST ) ) { fk_free( X ); fk_free( Y ); return( ATL_rzero ); }
#else
...
#ifdef IFKO
   fk_free( X );
   YD = (TYPE *)fk_malloc( ATL_MulBySize( N ) );
   if( YD == NULL ) { fk_free( Y ); return( ATL_rnone ); }
#else
   free( X );
...
#ifdef IFKO
   XD = (TYPE *)fk_malloc( ATL_MulBySize( N ) );
   if( XD == NULL ) { fk_free( Y ); fk_free( X ); return( ATL_rnone ); }
   YD = (TYPE *)fk_malloc( ATL_MulBySize( N ) );
   if( YD == NULL ) 
   {  
      fk_free( XD ); fk_free( Y ); fk_free( X ); 
      return( ATL_rnone ); 
   }
#else
   XD = (TYPE *)malloc( ATL_MulBySize( N ) );
...
#ifdef IFKO
   X  = (TYPE   *)fk_malloc( ATL_MulBySize( N ) * aincX );
   Y  = (TYPE   *)fk_malloc( ATL_MulBySize( N ) * aincY );
#else
   X  = (TYPE   *)malloc( ATL_MulBySize( N ) * aincX );
...
#ifdef IFKO
   X0 = (TYPE *)fk_malloc( ATL_MulBySize( lX ) );
   if( X0 == NULL ) { if( X0 ) fk_free( X0 ); return( -1 ); }
#else
...
)
{
   ...
   lX = N * aincX * ( ( ATL_DivBySize( LCSIZE ) + N - 1 ) / N );
#ifdef IFKO
   Y0 = (TYPE *)fk_malloc( ATL_MulBySize( lY ) );
   X0 = (TYPE *)fk_malloc( ATL_MulBySize( lX ) );
#else
