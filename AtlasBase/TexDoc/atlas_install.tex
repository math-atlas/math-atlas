...
\Wskip{
\subsubsection{Special Instructions for 32-bit Windows users}
\label{sec-windows32}
...
\begin{minipage}{6in}
{\bf
I have no idea if this still works.  Instructions from ATLASv3.8.
Will fix later.
}
\end{minipage}
}
...
\subsection{Special instructions for ARM with HARDFP ABIs}
ATLAS assumes that the ARM ABI used is so-called SOFTFP, where
floating-point values are passed in the integer registers.  If you
are using a OS that has switched to the HARDFP ABI, please scope
the errata file for additional installation instructions.

\subsection{Special instructions for OS~X}
Newer versions of OS~X ship broken versions of the gnu binutils.  They
are deprecated in favor of \textbf{clang}, which presently produces
...
\item [{\tt xatl2tvec}]: Reads in the output of a timer file, and produces
   ...
   in {\tt atlas\_tvec.h} and all downstream tools.
\item [{\tt xatlcat2tvec}]: Reads in the output of multiple concatonated ATLAS
      timer result files, assuming each individual run output is terminated with
      a line that looks like:
\begin{verbatim}
*** END LABEL=<lab> NREP=<#>
\end{verbatim}
      where \verb+<lab>+ is a label to be suffixed to all vector names
      for that run (to make them unique from other runs), and \verb+<#>+
      is the number of timing repetitions used in the preceeding timing run.
      It then produces one timing vector file with all vectors from
      all runs.
...
\item [{\tt xrenametvecs}]: rename an optional list of vectors, and if
      required suffix all input vectors with a string.
