...
in strongly reduced performance.  Unfortunately, Windows chose not to use
the standard AMD64 assembly ABI, and so ATLAS's assembly kernels cannot 
be used on 64-bit Windows.  Therefore, ATLAS's architectural defaults
cannot be used (because they include assembly files with the wrong ABI),
and ATLAS performance is noticably worse than it is under other OSes
(such as GNU/Linux or OS~X).
