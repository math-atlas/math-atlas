...
vice versa.  Therefore, the easiest way to support a particular is to
...
You can also use only the big or little cores for ATLAS parallel work,
by using {\tt --force-tids} (see \S\ref{sec-tids}), and specifying which
cores to use.  This will get a subset of cores running at the best rate
ATLAS can manage, and leave the other subset free for unrelated work.
...
The reason is that 3.10 uses a lot of statically scheduled algorithms,
...
factor used by the little cores.  

So, as an example, my odroid system has 4 a7 (little) cores and 4 a9
(bit) cores.  Assuming I want to use all 8 cores, my configure line
would be:
\begin{verbatim}
