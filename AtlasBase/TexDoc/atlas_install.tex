...
\begin{minipage}{6in}
{\bf
64-bit builds currently don't work for me.  Library production
will continue until the first big driver is run ({\tt xdfindCE} 
during the install).  This executable will fail to produce any output.
I believe it is a library issue that can probably be fixed with getting
all the MinGW libs rather than cygwin ones, but don't have time to 
investigate.
}
\end{minipage}
}
...
\item \verb+usage+: for lapack, choose `qr', `rq' `ql', `lq', `lu',
                    `lt' or `ut' (last two Cholesky) \\
...
If you want to get a complete comparison of ATLAS versus one library,
you can do this with:
\begin{verbatim}
   make <pre>[t,s]atlv<lib>_all
\end{verbatim}
Where `t' is threaded, `s' is serial, and \verb+<lib>+ is one of
{\tt sys} or {\tt at2}.  If the leading \verb+<pre>+ is omitted,
charts for all precisions will be built.
...
\begin{verbatim}
   make charts/<pre>mmla_<lib>[_pt].eps    # charts everything in raw MFLOP
   make charts/<pre>pcmm_<lib>[_pt].eps    # charts everything as % of GEMM
\end{verbatim}
You can also get a summary chart of all QR variants using:
\begin{verbatim}
   make charts/<pre>laqrs_<lib>[_pt].eps    # all QR variants as raw MFLOP
\end{verbatim}
Where \verb+<lib>+ is one of `atl', `sys', `at2'.
To build all precisions for these charts, you can issue:
\begin{verbatim}
   make mmla_<lib>[_pt]
   make pcmm_<lib>[_pt]
\end{verbatim}
